...
| `output/apresentacao_banca1/oferta_total_mensal.png` | a mesma série mensal, agregada, sem quebra por faixa: uma única linha, razão dos totais nos 295 municípios. Gerada em 20/09/2026. Carrega a limitação da figura por faixa e de `P8`: **sem grupo de comparação**, a subida não se lê como efeito do programa — a série cresce desde junho de 2024, antes de qualquer oferta. Entra apenas como contexto descritivo; a série está no `manifesto_figuras.json` sob `serie_total_por_100k` |
