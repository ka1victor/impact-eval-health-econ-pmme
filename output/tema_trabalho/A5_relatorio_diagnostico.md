...
No `delta_minimal` (variante primária de efeito fixo), a exclusão sucessiva de cada UF e de cada curso move o coeficiente de atração dentro de 0.235 a 0.940 sd 0.111; leave-one-município: base 0.506, faixa 0.391 a 0.623, com os três municípios mais influentes 355710 (Δ 0.12, DFBETA 0.47), 522045 (Δ -0.11, DFBETA -0.46), 355370 (Δ 0.07, DFBETA 0.28). Ver `A5_tabela_04_leave_one_out.csv` e `A5_tabela_05_influencia_municipal.csv`. Validação preditiva por município (GroupKFold, 5 dobras): `delta_minimal` com R² fora da amostra -0.315 contra 0.933 dentro; `estoque_6m_minimal` -0.078 contra 0.876. O R² dentro da amostra não deve ser lido como ajuste: 3 células ocupam níveis singleton de efeito fixo (41; MACRO_CENTRO-OESTE; MACRO_SUDESTE), que o próprio dummy ajusta exatamente e que respondem por 84.2% da soma de quadrados de `estoque_6m`. Sem elas, o R² dentro da amostra é 0.209 para `estoque_6m_minimal` e 0.119 para `delta_minimal`, sobre n efetivo 584 (item A-1b). Ver `A5_tabela_06_validacao_preditiva.csv` e `A5_tabela_15_singletons_efeito_fixo.csv`.
...
*Gerado em 2026-09-21 por `scripts/tema_trabalho/06_avaliar_provimento_cnes.py`.*
