...
with open("output/revisao_literatura/ranking_rubrica_papers_completo.json", "r", encoding="utf-8") as f:
    papers = json.load(f)

md_content = """# 09. Rúbrica Estratégica de Avaliação de Literatura e Seleção dos Top 7 Papers Teóricos
...
graph TD
    ...
    T --> T1["T1: Modelagem Formal de Decisão & Otimização (25 pts)"]
    T --> T2["T2: Aderência aos Mecanismos do PMM-E (25 pts)"]
    T --> T3["T3: Microfundamentação da Equação Estimada (25 pts)"]
    T --> T4["T4: Concisão & Digestibilidade Pedagógica (25 pts)"]
    
    E --> E1["E1: Rigor no Desenho de Identificação Causal (25 pts)"]
    E --> E2["E2: Aderência a Dados Administrativos / SUS (25 pts)"]
...
* **Pontuação Teórica:** T1 = 25 | T2 = 25 | T3 = 23 | T4 = 24  $\rightarrow$ **Total: 97/100**
...
* **Pontuação Teórica:** T1 = 24 | T2 = 24 | T3 = 24 | T4 = 25  $\rightarrow$ **Total: 97/100**
* **Conteúdo e Modelo Microeconômico:**
  - Aplica o Modelo de Utilidade Aleatória (RUM) de McFadden ao mercado de trabalho médico. A probabilidade de um médico $i$ escolher a especialidade $s$ no local $m$ é dada por:
    $$U_{ism} = \beta_w \cdot Salario_{sm} + \beta_h \cdot Horas_{sm} + \beta_{urb} \cdot Urbano_m + \varepsilon_{ism}$$
...
* **Pontuação Teórica:** T1 = 25 | T2 = 24 | T3 = 25 | T4 = 22  $\rightarrow$ **Total: 96/100**
* **Conteúdo e Modelo Microeconômico:**
  - Formulação rigorosa da função de produção hospitalar $Y = F(K, L, T)$ onde os hospitais minimizam custos sob regulação de preços. Mostra como mudanças no preço relativo do trabalho alteram a taxa marginal de substituição técnica entre médicos e capital hospitalar:
    $$\frac{F_K(K, L, T)}{F_L(K, L, T)} = \frac{r_K}{w_L}$$
* **Por que é crucial para o PMM-E:**
...
* **Pontuação Teórica:** T1 = 24 | T2 = 25 | T3 = 25 | T4 = 22  $\rightarrow$ **Total: 96/100**
* **Conteúdo e Modelo Microeconômico:**
  - Modela o comportamento oportunista do gestor público local que maximiza utilidade orçamentária sob transferências federais vinculadas. Demonstra as condições matemáticas sob as quais ocorre *crowding-out* fiscal:
    $$\frac{\partial GastosLocais}{\partial TransfFederal} < 0$$
* **Por que é crucial para o PMM-E:**
...
* **Pontuação Teórica:** T1 = 25 | T2 = 25 | T3 = 24 | T4 = 21  $\rightarrow$ **Total: 95/100**
* **Conteúdo e Modelo Microeconômico:**
...
* **Pontuação Teórica:** T1 = 23 | T2 = 25 | T3 = 22 | T4 = 23  $\rightarrow$ **Total: 93/100**
* **Conteúdo e Modelo Microeconômico:**
...
* **Pontuação Teórica:** T1 = 23 | T2 = 22 | T3 = 24 | T4 = 24  $\rightarrow$ **Total: 93/100**
* **Conteúdo e Modelo Microeconômico:**
...
* **Por que é crucial para o PMM-E:**
  - Justifica matematicamente a nossa especificação canônica $Y_{mst} = \alpha_{ms} + \gamma_{mt} + \delta_{st} + \beta (\text{Immediate}_{ms} \times \text{Post}_t) + \varepsilon_{mst}$, blindando a nota técnica contra críticas metodológicas.
...
with open("docs/09_rubrica_estrategica_e_selecao_teorica.md", "w", encoding="utf-8") as f:
    f.write(md_content)
